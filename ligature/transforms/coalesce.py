...
class Coalesce(Transform):
    ...
    __slots__ = ('_key_field', '_coalesce_field', 
                 '_target_fields', '_scanner_coverage',)
    
    ScanClass = RecordScanner
    
    def __init__(self, sources, key_field, target_fields=None, coalesce_field=None):
        super(Coalesce, self).__init__()
        self.sources = tuple(sources)
        
        self._key_field = key_field
        self._coalesce_field = coalesce_field
    ...
    def _resolveSources(self):
        ...
        for source in rawSources:
            ...
            for field in source._RecordType._fields:
                if field in (self._key_field, self._coalesce_field,):
                    continue
        ...
        self._resultSet = RecordSet(recordType=((self._key_field,) + self._target_fields + ((self._coalesce_field,) or tuple())))
    ...
    def transform(self):
        ...
        while cursor_value_heap:
            ...
            if self._coalesce_field:
                group_value = entry[self._coalesce_field]

                # when grouping for merge, assume group final value is most recent by key sort value
                if cursor_values[self._coalesce_field] == group_value:
                    merged[-1] = self._resultSet._RecordType(cursor_values)
                else:
                    cursor_values[self._coalesce_field] = group_value
